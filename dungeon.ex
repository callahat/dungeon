...
# Mainly implemented with charlists, which is probably why it seems to run a bit slow
defmodule Dungeon do
  ...
  def generate() do
    map = String.duplicate(" ", @cave_height * @cave_width) |> to_charlist
  ...
  defp _tile_at(map, col, row) do
    Enum.at(map, row * @cave_width + col)
  end

  defp _replace_tile_at(map, col, row, new_tile) do
    # IO.puts "#{col} #{row} #{[new_tile]}"
    List.replace_at(map, row * @cave_width + col, new_tile)
  end
  ...
  defp _door_candidates(map, coords = %{top_left_col: tlc, top_left_row: tlr, bottom_right_col: brc, bottom_right_row: brr}) do
    ...
    |> Enum.concat
    |> Enum.concat
    |> Enum.filter(fn({col, row}) -> Enum.at(map, row * @cave_width + col) == ?# end)
  end
